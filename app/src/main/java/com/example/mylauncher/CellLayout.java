...
public class CellLayout extends ViewGroup {
    ...
    public CellLayout(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.CellLayout, 0, 0);
        mCellWidth = a.getDimensionPixelSize(R.styleable.CellLayout_cellWidth, 24);
        mCellHeight = a.getDimensionPixelSize(R.styleable.CellLayout_cellHeight, 24);

        mLongAxisStartPadding =
                a.getDimensionPixelSize(R.styleable.CellLayout_longAxisStartPadding, 0);
        mLongAxisEndPadding =
                a.getDimensionPixelSize(R.styleable.CellLayout_longAxisEndPadding, 0);
        mShortAxisStartPadding =
                a.getDimensionPixelSize(R.styleable.CellLayout_shortAxisStartPadding, 0);
        mShortAxisEndPadding =
                a.getDimensionPixelSize(R.styleable.CellLayout_shortAxisEndPadding, 0);

        mShortAxisCells = a.getInt(R.styleable.CellLayout_shortAxisCells, 0);
        mLongAxisCells = a.getInt(R.styleable.CellLayout_longAxisCells, 0);
        a.recycle();
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            if (!found) {
                ...
                Log.e("CellLayout", "mOccupied="+ mOccupied.length+"mOccupied ="+mOccupied[0].length+"xcount="+xCount+"ycont="+yCount);
                ...
                cellInfo.spanX = 1;
                cellInfo.spanY = 1;
                Log.e("CellLayout", cellXY[0]+" "+cellXY[1]);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        calculateShortAxisCellsAndLongAxisCells();
        ...
        Log.d(getClass().getName(),"shortAxis="+mShortAxisCells+", longAxis="+mLongAxisCells);
    ...
    public int[] coordinateToCellXY(float x1,float y1,int leftMargin,int topMargin){
        int[] cell=new int[2];
        if (mPortrait) {
            cell[0]=((int)x1-mShortAxisStartPadding-leftMargin)/(mCellWidth+mWidthGap);
            cell[1]=((int)y1-mLongAxisStartPadding-topMargin)/(mCellHeight + mHeightGap);
        } else {
            cell[0]=((int)x1-mLongAxisStartPadding-leftMargin)/(mCellWidth+mWidthGap);
            cell[1]=((int)y1-mShortAxisStartPadding-topMargin)/(mCellHeight + mHeightGap);
        }
        Log.d(getClass().getName(),"cellX="+cell[0]+"CellY="+cell[1]);
        return cell;
    }
    /**
    ...
    public void calculateShortAxisCellsAndLongAxisCells(){
        boolean check=mShortAxisCells==0||mLongAxisCells==0;
        if (mPortrait){
            if(check){
                DisplayMetrics displayMetrics = getResources().getDisplayMetrics();
                if (mShortAxisCells == 0) {
                    int cellWidth = getCellWidth();
                    float screenWidthDp = displayMetrics.widthPixels;
                    int noOfColumns=(int) (screenWidthDp / cellWidth + 0.5);
                    this.setShortAxisCells(noOfColumns);
                }
                if (mLongAxisCells == 0){
                    int cellHeight = (getCellHeight());
                    float screenHeightDp = displayMetrics.heightPixels-90*displayMetrics.density ;
                    int noOfRows=(int) (screenHeightDp / cellHeight + 0.5);
                    this.setLongAxisCells(noOfRows);
                }
            }
            ...
            if(check) {
                ...
                if (mLongAxisCells == 0){
                    ...
                    float screenWidthDp = displayMetrics.widthPixels;
                    int noOfColumns = (int) (screenWidthDp / cellWidth + 0.5);
                    this.setLongAxisCells(noOfColumns);
                }
                if (mShortAxisCells == 0){
                    float screenHeightDp = displayMetrics.heightPixels;
                    int cellHeight = getCellHeight();
                    int noOfRows = (int) (screenHeightDp / cellHeight + 0.5);
                    this.setShortAxisCells(noOfRows);
                }
            }
            if (mOccupied == null)
    ...
    public void setPortrait(boolean mPortrait) {
    ...
    public void setShortAxisCells(int mShortAxisCells) {
        this.mShortAxisCells = mShortAxisCells;
    }
    ...
    public void setLongAxisCells(int mLongAxisCells) {
        this.mLongAxisCells = mLongAxisCells;
    }
}